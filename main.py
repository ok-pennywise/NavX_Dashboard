...
class TempGauge(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Color Logic
        if self.temp > 100: color = QColor("#FF0000") # Danger
        elif self.temp > 80: color = QColor("#FFA500") # Warning
        else: color = QColor("#00E5FF") # Normal
...
class NavXDashboard(QMainWindow):
    ...
    def init_ui(self):
        ...
        # HEADER
        header = QHBoxLayout()
        self.batt_bar = self.create_bar("ENERGY", "#00FF66")
    ...
    def keyPressEvent(self, event):
        key = event.key()
        if key == Qt.Key.Key_E and self.gear_status == "N":
    ...
    def update_dashboard(self):
        ...
        self.stack.setCurrentIndex(1 if self.is_engine_running else 0)
